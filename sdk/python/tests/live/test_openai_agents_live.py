"""
Live integration tests for FiGuard OpenAI Agents SDK integration.

These tests exercise guarded_function_tool against a real running figuard-core
container. No actual OpenAI Agents runs are performed — functions are called
directly to verify the FiGuard authorize → execute → confirm lifecycle.

Run:
    make run                   # start figuard-core container
    pytest tests/live/test_openai_agents_live.py -v
"""
...
pytest.importorskip(
    "agents",
    reason="openai-agents not installed — pip install figuard[openai-agents]",
)
...
# ---------------------------------------------------------------------------
# guarded_function_tool — live tests
# ---------------------------------------------------------------------------

class TestGuardedFunctionToolLive:
    ...
    def test_authorized_call_executes_and_confirms(
        ...
        @guarded_function_tool(
            ...
            agent_id="live_oai_agents_agent",
        )
        def book_hotel(city: str, amount: float) -> str:
            return f"Hotel in {city} booked"

        result = book_hotel(city="Chicago", amount=120.0)

        assert result == "Hotel in Chicago booked"
    ...
    def test_allocation_category_match_authorizes(
        ...
        result = book_flight(destination="SEA", price=210.0)
    ...
    def test_tool_error_fails_event_and_reraises(
        ...
        def broken_tool(amount: float) -> str:
            raise RuntimeError("openai agents tool failure")

        with pytest.raises(RuntimeError, match="openai agents tool failure"):
    ...
    def test_custom_amount_key(
        ...
        result = book_flight(destination="BOS", price=175.0)
    ...
    def test_allocation_exhausted_after_limit_reached(self, client: FiGuardClient):
        """
        After an allocation is exhausted, further calls are denied.
        """
